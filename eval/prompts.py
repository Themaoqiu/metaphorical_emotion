...
SYSTEM_PROMPT = """You are an expert visual emotion analyst tasked with understanding emotions based on visual content.
...
Based on visual information, judge whether there is a metaphor in the image. If there are metaphors in the image content, think about which comprehension path to use and further analyze the reasons within <metaphor></metaphor>. If there is no metaphor in the image, output <metaphor>There is no metaphor</metaphor>.
...
MULTIMM_USER_PROMPT_SUFFIX = """
Please analyze the image emotion carefully by identifying key elements and metaphor in the image within `<caption> </caption>`, `<metaphor> </metaphor>`, `<think> </think>` tags then conduct deep analysis and reasoning to arrive at your answer to the question, finally provide only the single emotion(among positive, neutral and negative) within the `<answer> </answer>` tags. Follow the format specified in the instructions."""
...
def build_user_prompt(question: str) -> str:
    return f"{question}{MULTIMM_USER_PROMPT_SUFFIX}"


def parse_metaphor_label(response: str) -> int:
    match = re.search(r"<metaphor>(.*?)</metaphor>", response, flags=re.IGNORECASE | re.DOTALL)
    content = match.group(1).strip().lower() if match else response.lower()
    if "no metaphor" in content:
        return 0
    return 1


def _normalize_answer_text(response: str) -> str:
    match = re.search(r"<answer>(.*?)</answer>", response, flags=re.IGNORECASE | re.DOTALL)
    content = match.group(1) if match else response
    return re.sub(r"[^a-z]+", " ", content.lower()).strip()


def parse_emotion_to_sentiment_label(response: str) -> int:
    tokens = set(_normalize_answer_text(response).split())
